...
static void update_variables(void)
{
   ...
   var.key = "hatari_joypad_B";
   var.value = NULL;

   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
	   if(strcmp(var.value, "autofire") == 0)joybconfig=0;
	   else if (strcmp(var.value, "control") == 0)joybconfig=1;
	   else if (strcmp(var.value, "space") == 0)joybconfig=2;

   }

   var.key = "hatari_joypad_R2";
   var.value = NULL;

   if (environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value)
   {
	   if(strcmp(var.value, "1") == 0)joyr2config=0;
	   else if (strcmp(var.value, "y") == 0)joyr2config=1;
	   else if (strcmp(var.value, "n") == 0)joyr2config=2;
	   else if (strcmp(var.value, "j") == 0)joyr2config=3;
	   else if (strcmp(var.value, "s") == 0)joyr2config=4;
   }
...
static struct retro_input_descriptor input_descriptors[] = {
   ...
   { 0, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_A, "Fire /mouse b1" },
   { 0, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_B, "Button opt config 1/mouse b2" },
   { 0, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_X, "Shift ON/OFF" },
   ...
   { 0, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_START, "Toggle num joy" },
   { 0, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_L2, "Toggle m/k status" },
   { 0, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_L, "show/hide vkbd" },
   { 0, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_R2, "Button opt config 2" },
...
#define GIT_VERSION ""
#endif
   info->library_version  = "2.2.1";// GIT_VERSION;
...
void retro_run(void)
{
   ...
   if (CHANGE_RATE || CHANGEAV_TIMING || UPDATE_GEOM)
   {
