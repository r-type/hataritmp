...
printf("(%dx%d) (%d,%d) %d\n",pSdlGuiScrn->w,pSdlGuiScrn->h,retrow,retroh,pSdlGuiScrn->pitch);
	sdlgui_fontwidth  = 10;
...
extern unsigned char mfont[font10x16_height*font10x16_width];
...
void draw_font(int x,int y,int sx,int sy) {

	int i,j,xx;
	int si,sj;
	int dx=sdlgui_fontwidth,dy=sdlgui_fontheight;
	...
	for(j=y,sj=sy;j<y+dy;j++,sj++){
		...
		for(i=x,si=sx;i<x+dx;i++,si++){
			...
			if(mfont[(sj)*font10x16_width+(si)]==1)
			{
				DrawPointBmp(bmp,i,j, 0);
 ...
 */
static void SDLGui_ScaleMouseButtonCoordinates(int/*SDL_MouseButtonEvent*/ *bev)
{
#if WITH_SDL2
...
*/
		...
		if(retbutton ==SDLGUI_NOTFOUND)retbutton = 1;
//		}
